...
sub start
{
    ...
    $self->log->info('[Sybil] [BUILD] New session beginning ' .  localtime(time));
    $self->discord->init();
...
{
    my ($self, $user) = @_;
    $self->log->info('[Sybil] [_add_me] My Discord ID: ' . $user->{'id'});
...
sub add_command
{
    ...
    $self->log->debug('[Sybil] [add_moo_command] Registered new command: "' . $name . '" identified by "' . $command->pattern . '"');
}
